...
def is_valid_name(name: str) -> bool:
    return name.strip() != "" and len(name.strip()) < 10 and " " not in name

# 상담사 스타일 인트로 생성
def get_counselor_intro(name: str, issue: str, style: str) -> str:
    intro_name = f"{name}님, " if is_valid_name(name) else ""
    if style == "공감형":
        return f"{intro_name}'{issue}'에 대해 이야기해 주셔서 감사합니다. 저는 공감적으로 경청하며 함께 고민을 나눌게요."
    elif style == "조언형":
        return f"{intro_name}'{issue}' 문제, 같이 현실적으로 정리해봅시다. 제가 솔직하게 조언드릴게요."
    elif style == "유머러스형":
        return f"{intro_name}'{issue}'이요? 너무 심각하게만 생각 말아요! 우리 가볍게 풀면서 이야기해볼까요?"
    else:
        return f"{intro_name}'{issue}'에 대해 이야기해 주세요."

# 프롬프트 구성
def build_prompt(data: ChatRequest) -> str:
    valid_name = data.name if is_valid_name(data.name) else "비공개"
    intro = f"""상담 시작 전 정보:
이름: {valid_name}
성별: {data.gender}
